...
## 版本说明
本文中的安装目标版本，基于 `2026-03-08` 当天的官方信息。
...
### 3. 查询可用的 `JDK 21` 发行版标识
由于 `SDKMAN` 中不同供应商的 JDK 标识可能会随时间变化，正式安装前建议先查一下：
```bash
sdk list java | rg '21.*tem|21.*zulu|21.*librca|21.*oracle'
```

如果你只是想选一个通用、稳定的发行版，通常可以优先看 `Temurin`。

### 4. 安装 `JDK 21`
如果查询结果里仍然有 `21-tem`，可以直接这样安装：
```bash
...
如果 `21-tem` 不存在，就把上面的标识替换成你在 `sdk list java` 中实际看到的 `21` 对应发行版标识。
...
### 6. 如果 `SDKMAN` 中安装 Maven 失败，则改用 Apache 官方压缩包
```bash
cd /tmp
...
sudo ln -sfn /opt/apache-maven-3.9.12 /opt/maven
...
## 如果你已经安装了其他 JDK，如何切换到 JDK 21
如果你现在已经装了 `JDK 25` 或其他版本，不一定要先卸载。
可以直接这样切换：
...
如果你只想临时在当前 shell 中切换，可以把 `sdk default` 换成：
```bash
sdk use java 21-tem
```
...
3. 在 WSL Ubuntu 中安装 `JDK 21`
4. 在 WSL Ubuntu 中安装 `Maven 3.9.12`
5. 验证 `java`、`mvn`、`docker` 是否都可用
6. 只有在真实遇到网络问题时，再处理 WSL 侧代理配置

## 当前官方基线信息
截至 `2026-03-08`：
- Spring Boot 官方博客显示 `3.5.11` 已发布
- Spring 官方支持策略说明 `3.5` 作为 `3.x` 的最后一个 minor version 享有额外的企业支持周期
- Oracle 官方路线图显示 `Java 21` 和 `Java 25` 都是 LTS，其中 `21` 更适合作为当前学习主线的稳态基线
- Apache Maven 官方下载页显示 `3.9.12` 为当前稳定版
