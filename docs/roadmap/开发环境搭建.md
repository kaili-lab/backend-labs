...
后续数据库、Redis，以及进入微服务项目后可能需要的消息队列等依赖统一由 Docker 管理。
...
### 3. 不需要提前安装 Kafka
当前 Spring Boot 单体项目不包含 Kafka。
等进入 `mall-microservices/` 再决定是否拉起 Kafka 相关容器和 CLI。
...
### 5. 不需要提前安装全部中间件客户端
例如 `psql`、Redis CLI 等工具，只有在进入对应阶段且确实需要调试时再补充即可。
`Kafka CLI` 也应等进入 `mall-microservices/` 后再考虑。
...
- Docker Compose 负责数据库、Redis，以及后续微服务阶段依赖的编排
